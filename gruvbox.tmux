...
#+--- Bars LEFT ---+
# Session name
tmux set -g status-left "#[fg=${THEME[foreground]},bg=${THEME[blue]},bold] #{?client_prefix,󰠠 ,#[dim]󰤂 }#[bold,nodim]#S "

#+--- Windows ---+
# Focus
tmux set -g window-status-current-format "$RESET#[fg=${THEME[green]},bg=${THEME[black]}] #{?#{==:#{pane_current_command},ssh},󰣀 , }#[fg=${THEME[magenta]},bold,nodim]$window_number#W#[nobold]#{?window_zoomed_flag, $zoom_number, $custom_pane}#{?window_last_flag, , }"
# Unfocused
...
#+--- Bars RIGHT ---+
tmux set -g status-right "$git_status$wb_git_status$current_path$cmus_status$battery_status$netspeed$date_and_time"
